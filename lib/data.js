...
export async function createCollection(input) {
  ...
  await query(
    `INSERT INTO collections (id, name, price_cents, includes, discounts, position, active)
...
export async function updateCollection(id, input) {
  await query(
    `UPDATE collections SET name = $2, price_cents = $3, includes = $4,
       discounts = $5, position = $6, active = $7 WHERE id = $1`,
    [
      id,
      input.name,
      input.priceCents,
...
export async function createAddon(input) {
  ...
  await query(
    `INSERT INTO addons (id, name, unit_price_cents, description, position, active)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [
      id,
...
export async function updateAddon(id, input) {
  await query(
    `UPDATE addons SET name = $2, unit_price_cents = $3, description = $4,
       position = $5, active = $6 WHERE id = $1`,
    [id, input.name, input.unitPriceCents, input.description, input.position, input.active]
  );
}
...
export async function createQuote({ clientName, clientEmail }) {
