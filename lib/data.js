...
export async function createAddon(input) {
  ...
  await query(
    `INSERT INTO addons (id, name, unit_price_cents, description, position, active, category)
...
export async function updateAddon(id, input) {
  await query(
    `UPDATE addons SET name = $2, unit_price_cents = $3, description = $4,
       position = $5, active = $6, category = $7 WHERE id = $1`,
    [
      id,
      input.name,
...
export async function createQuote({ clientName, clientEmail, quoteType }) {
  ...
  const items = [
    ...collections
      .filter((c) => c.active && matches(c.category))
      .map((c) => ({
        key: newId(),
        kind: 'collection',
        name: c.name,
    ...
    ...addons
      .filter((a) => a.active && matches(a.category))
      .map((a) => ({
        key: newId(),
        kind: 'addon',
        name: a.name,
