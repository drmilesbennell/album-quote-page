...
export async function createAddon(input) {
  ...
  await query(
    `INSERT INTO addons (id, name, unit_price_cents, description, position, active, category, parent_collection_id)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
    [
      id,
...
export async function updateAddon(id, input) {
  await query(
    `UPDATE addons SET name = $2, unit_price_cents = $3, description = $4,
       position = $5, active = $6, category = $7, parent_collection_id = $8 WHERE id = $1`,
    [
      id,
      input.name,
