...
const SEED_ADDONS = [
  // [id, name, unitPriceCents, description, category]
  ['seed-pages', 'Additional Album Pages', 4000, 'A la carte, per page', 'both'],
  ['seed-pages-keepsake', 'Additional Album Pages - Keepsake', 3500, 'Per page with the Keepsake collection', 'wedding'],
  ['seed-pages-heirloom', 'Additional Album Pages - Heirloom', 3000, 'Per page with the Heirloom collection', 'wedding'],
  ['seed-pages-legacy', 'Additional Album Pages - Legacy', 2500, 'Per page with the Legacy collection', 'wedding'],
...
async function seedIfEmpty(query) {
  ...
  position = 0;
  for (const [id, name, price, description, category] of SEED_ADDONS) {
    position += 1;
    await query(
      `INSERT INTO addons (id, name, unit_price_cents, description, position, category)
       VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO NOTHING`,
      [id, name, price, description, position, category]
    );
  }
}
